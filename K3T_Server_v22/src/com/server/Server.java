...
public class Server {
    ...
    private static class Handler extends Thread {
        ...
        private Message sendRequestToFriend(Message inputmsg) throws IOException {
            ...
            msg.setType(MessageType.CONNECT_FRIEND);
            msg.setUserlist(names);
            msg.setUsers(users);
         ...
         //Ham gui yeu cau ket noi cua user A den user B thong qua server center
        private Message sendStatusConnect(Message inputmsg) throws IOException {
            String userNeedSendNotice = inputmsg.getMsg();// User name B ma user A muon gui trang thai ket noi den
            String userSendNotice = inputmsg.getName();// User name A thong bao trang thai ken noi den server center
            Message msg = new Message();
            msg.setName("SERVER CENTER");            
            msg.setMsg(userSendNotice);
            msg.setType(MessageType.STATUS_CONNECT);
            msg.setUserlist(names);
            msg.setUsers(users);
            msg.setOnlineCount(names.size());            
            msg.setIp(inputmsg.getIp());//Trang thai ket noi
            msg.setPort("");
            
            //Tim ObjectOutputStream cua user B
            if (writers.containsKey(userNeedSendNotice)) {//Kiem tra co hay khong user B
                ObjectOutputStream writer = writers.get(userNeedSendNotice);
                writer.writeObject(msg);
                writer.reset();
            }
            else{
                logger.info("No user: "+ userNeedSendNotice);
            }
            return msg;
        }       
